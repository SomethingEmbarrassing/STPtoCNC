...
def _find_part_mark(text: str) -> str | None:
    match = re.search(r"\b(?:PART\s*MARK|PIECE\s*MARK|MARK)\s*[:=]\s*([A-Za-z0-9_.-]+)", text, flags=re.IGNORECASE)
    return match.group(1) if match else None


def _infer_profile_family(profile_line: str | None) -> ProfileFamily:
    if not profile_line:
        return ProfileFamily.UNKNOWN
    upper = profile_line.upper()
    if upper.startswith("PIPE"):
        return ProfileFamily.PIPE
    if upper.startswith("HSS"):
        return ProfileFamily.HSS
    if re.match(r"^L\d", upper):
        return ProfileFamily.ANGLE
    return ProfileFamily.UNKNOWN
...
def _extract_quantity(non_empty_lines: list[str], profile_line: str | None) -> tuple[int, str]:
    """Use NC1 quantity when present, else fallback to default=1."""
    if profile_line and profile_line in non_empty_lines:
        profile_idx = non_empty_lines.index(profile_line)
        for idx in range(profile_idx - 1, max(-1, profile_idx - 6), -1):
            token = non_empty_lines[idx].strip()
            if re.fullmatch(r"\d+", token):
                return max(1, int(token)), "nc1"
    return 1, "default"


def _parse_tekla_style_fallback(text: str) -> dict[str, float | str | int | ProfileFamily | None]:
    """Extract values from unlabeled Tekla-like NC1 blocks as fallback."""
    lines = [line.rstrip() for line in text.splitlines()]
    non_empty = [line.strip() for line in lines if line.strip()]

    part_mark = next((line for line in non_empty if re.fullmatch(r"[A-Za-z]{1,4}\d{2,}", line, flags=re.IGNORECASE)), None)
    profile_line = next((line for line in non_empty if re.search(r"^(PIPE|HSS|L\d)", line, flags=re.IGNORECASE)), None)
    profile_family = _infer_profile_family(profile_line)
    quantity, quantity_source = _extract_quantity(non_empty, profile_line)
    material = next((line for line in non_empty if re.search(r"\bGR\b|A\d{2,}", line, flags=re.IGNORECASE)), None)
...
def parse_nc1_text(text: str, source_path: str | None = None) -> Nc1Part:
    ...
    part_mark = _find_part_mark(text) or fallback["part_mark"]

    od = _find_first_float(text, [rf"\b(?:OD|OUTER\s*DIAM(?:ETER)?)\s*[:=]\s*{_FLOAT}", rf"\bDIA\s*[:=]\s*{_FLOAT}"])
    wall = _find_first_float(text, [rf"\b(?:WALL(?:\s*THICKNESS)?|THK|T)\s*[:=]\s*{_FLOAT}"])
    length = _find_first_float(text, [rf"\b(?:LENGTH|LEN|L)\s*[:=]\s*{_FLOAT}"])
